...
Widget forecastView(
    BuildContext context, int index, WeatherContentState state) {
  if (index == 0) {
    return Column(
    ...
    if (getDateFromTimestamp(state.weather.forecast[index - 1].dt,
            state.weather.currentWeather.timezone) ==
        getDateFromTimestamp(state.weather.forecast[index].dt,
            state.weather.currentWeather.timezone)) {
      return Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: <Widget>[
